...
import { mockQuizzes } from '../../../data/analyticsMockData';
  ...
  return (
    <div>
      ...
      <div className="d-flex flex-wrap gap-3 mb-4">
        <StatCard icon={HelpCircle} value="3" label="Total Quizzes" iconColorClass="text-primary" iconBgClass="bg-primary bg-opacity-10" />
        <StatCard icon={CheckCircle} value="2" label="Active" iconColorClass="text-success" iconBgClass="bg-success bg-opacity-10" />
        <StatCard icon={Clock} value="75%" label="Avg Pass Score" iconColorClass="text-warning" iconBgClass="bg-warning bg-opacity-10" />
      </div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div className="text-muted small">Showing 3 quizzes</div>
      </div>
          ...
          <table className="table table-hover mb-0 align-middle">
            <thead className="bg-light border-bottom">
              <tr>
                ...
                <th className="text-muted small fw-bold text-uppercase border-bottom-0 py-3 text-center lms-font-xs lms-tracking-wide">Questions</th>
                ...
                <th className="text-muted small fw-bold text-uppercase border-bottom-0 py-3 lms-font-xs lms-tracking-wide">Created</th>
            ...
            <tbody>
              {mockQuizzes.map((quiz, idx) => (
                <tr key={idx}>
                  <td className="ps-4 py-3 fw-bold text-dark small">{quiz.title}</td>
                  <td className="py-3 text-muted small">{quiz.course}</td>
                  <td className="py-3 fw-bold text-dark small text-center">{quiz.questions}</td>
                  <td className="py-3 text-center">
                    <Badge variant="success" className="bg-opacity-25 text-success fw-bold px-2 py-1 lms-font-xs">
                      {quiz.passScore}%
                    </Badge>
                  </td>
                  <td className="py-3 text-muted small">{quiz.timeLimit}</td>
                  <td className="py-3">
                    <Badge variant={quiz.status === 'Active' ? 'success' : 'secondary'} className="bg-opacity-10 fw-medium">
                      {quiz.status}
                    </Badge>
                  </td>
                  <td className="py-3 text-muted small">{quiz.created}</td>
